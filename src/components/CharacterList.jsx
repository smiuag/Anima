...
export default function CharacterList({ onSelect }) {
  ...
  return (
    <div className="min-h-screen bg-[#1d2a3e] flex flex-col">
      {/* Header */}
      <div className="bg-[#162030] border-b border-[#3a5070] px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-[#f5b832] text-2xl font-bold tracking-wider">⚔ ANIMA</h1>
          <p className="text-[#7fa8cc] text-xs mt-0.5">Gestor de Fichas de Personaje</p>
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="panel p-6 w-96">
            <h2 className="text-[#f5b832] font-bold text-lg mb-4">Nuevo Personaje</h2>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="panel p-6 w-96">
            ...
            <p className="text-[#7fa8cc] text-sm mb-4">
              Esta acción no se puede deshacer. ¿Seguro que quieres eliminar a <strong className="text-[#dce8f5]">{confirmDelete.nombre || 'este personaje'}</strong>?
            </p>
      ...
      <div className="flex-1 p-6">
        {characters.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {characters.map(char => (
              <div key={char.id} className="panel hover:border-[#f5b832] transition-colors cursor-pointer group"
                onClick={() => onSelect(char.id)}>
                <div className="p-4">
                  {/* Avatar / iniciales */}
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-12 h-12 rounded bg-[#1e2d45] border border-[#3a5070] flex items-center justify-center text-[#f5b832] font-bold text-lg flex-shrink-0">
                      {(char.nombre || '?')[0].toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-[#dce8f5] font-bold text-sm truncate group-hover:text-[#f5b832] transition-colors">
                        {char.nombre || 'Sin nombre'}
                      </h3>
                      <p className="text-[#7fa8cc] text-xs truncate">
                        {char.categoria || 'Sin categoría'}
                        {char.nivel > 0 && ` · Nv. ${char.nivel}`}
                      </p>
                      {char.raza && (
                        <p className="text-[#3a5070] text-xs">{char.raza}</p>
                      )}
                    </div>
                  </div>
                    ...
                    {['AGI', 'CON', 'FUE', 'VOL'].map(stat => {
                      ...
                      return (
                        <div key={stat} className="text-center">
                          <div className="text-[#3a5070] text-xs">{stat}</div>
                          <div className="text-[#f5b832] font-bold text-sm">{val}</div>
                        </div>
                      )
                    })}
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-[#7fa8cc]">
                      PV: <span className="text-[#dce8f5]">{char.puntosVida?.total || 20}</span>
                    </span>
                ...
                {/* Acciones */}
                <div className="border-t border-[#1e2d45] px-3 py-2 flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity"
                  onClick={e => e.stopPropagation()}>
                  <button className="text-[#7fa8cc] hover:text-[#f5b832] text-xs transition-colors"
      ...
      {/* Footer */}
      <div className="border-t border-[#1e2d45] px-6 py-2 text-center text-[#3a5070] text-xs">
